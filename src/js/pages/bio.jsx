  ...
  return (
    <div key={i} className="position">
      <div className="position__column1">
        ...
        <div className="position__date">{$m(p.startDate).format("YYYY")}</div>
      </div>
      <div className="position__column2">
        <div className="position__name">{position}</div>
        <div className="position__institution">{p.company}</div>
        <div className="position__department">- {p.address}</div>
      </div>
    </div>
  );
}
  ...
  return (
    <div key={i} className="diploma">
      <div className="diploma__column1">
        <div className="diploma__name">{p.studyType}</div>
        <div className="diploma__date">{$m(p.endDate).format("YYYY")}</div>
      </div>
      <div className="diploma__column2">
        <div className="diploma__topic">{p.area}</div>
        <div className="diploma__institution">{p.institution}</div>
        <div className="diploma__department">- {p.department}</div>
      </div>
    </div>
  );
}
  ...
  return (
    <div key={i} className="grant">
      <div className="grant__column1">
        <div className="grant__date">{$m(p.to).format("YYYY")}</div>
        <div className="grant__date">{$m(p.from).format("YYYY")}</div>
      </div>
      <div className="grant__column2">
        <div className="grant__role">{p.role}</div>
        <div className="grant__name">{p.project}</div>
        <div className="grant__institution">
          {p.institution} - {p.type}
        </div>
        <div className="grant__department">{p.department}</div>
      </div>
    </div>
  );
}
...
function renderGrantBlock($) {
  ...
  return (
    <div className="grants">
      ...
      <div className="grants-list">{_.map(grants, renderGrant)}</div>
    </div>
  );
}
...
function renderEducationBlock($) {
  return (
    <div className="education education_size_small">
      ...
      <div className="diploma-list">{_.map($.education, renderDiploma)}</div>
    </div>
  );
}

function renderCareerBlock($) {
  let workSorted = _.sortBy($.work.previous, x => -1 * $m(x.startDate).unix());
  return (
    <div className="career career_size_small">
      <div className="career__title"> Career </div>
      <div className="career-list">{workSorted.map(renderPosition)}</div>
    </div>
  );
}
        ...
        <div className="bio-container">
          <div className="bio">
            ...
            <img
              ...
              src={require("../../img/profile_pic_1_res.jpg")}
            />
